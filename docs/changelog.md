...
- Added a simpler Sage appearance and made buttons follow the selected palette.
- Published content now changes the worker fingerprint; drafts never enter runtime indexes.
- Live list refresh preserves filters; open details retain an explicit refresh action.
- Added optional configured responsive ad slots across shared page shells.
- Tightened content schemas, expanded the project template, fixed URL fragment handling and mixed section fields, and added detail navigation.
- Added isolated release and worker transaction regressions plus expanded Chromium coverage.

# Changelog

## v14.0.1 — Project presentation polish

- strengthened project-card accents, detail-page hierarchy, and list rendering
- made optional and legacy project content render safely
- expanded the Sentinel Network Monitor case study and removed placeholder links
- removed an accidental duplicate Network Analyzer record

## v14.0.0 — Visual identity refresh

- redesigned the site around the blue, teal, and green Guang identity artwork
- added the profile artwork to the home hero with responsive framing and accessible alt text
- introduced luminous surfaces, layered backgrounds, modernized cards, stronger typography, and refined dark mode
- added subtle intersection-based reveal motion with reduced-motion support
- refreshed archive, article, project-detail, navigation, button, filter, interest, and footer styling

## v13.3.0 — Separate content records

- split every project and article into its own canonical JSON file
- moved browser-only aggregate output into `data/catalogs/`
- added per-record schemas, filename/ID checks, source/catalog drift checks, and stricter required-field validation
- updated content creation, rendering, service-worker paths, documentation, and release validation

## v13.2.0 — Repository organization

- consolidated site configuration, schemas, templates, catalogs, and drafts under `data/`
- moved deployment, content-model, template, and changelog documentation under `docs/`
- removed the obsolete one-time legacy import path from the build script
- updated service-worker paths, validation, authoring tools, robots rules, and repository documentation

## v13.1.0 — Safer catalog authoring and updates

- added JSON Schemas and catalog `$schema` links for editor validation and autocomplete
- added draft/published visibility controls and featured metadata to every record
- added stable catalog content hashes and a lightweight update manifest
- fixed renderer compatibility with both `content`/`items` and `paragraphs`/`points` section formats
- added route and publication-filter tests to the release pipeline

## v13.0.0 — Data-driven publishing

- replaced 14 generated detail pages with two client-rendered collection shells
- consolidated project and article sources into `data/projects.json` and `data/articles.json`
- added bookmarkable `?id=` routes plus redirects for legacy `.html` bookmarks
- added automatic catalog refresh checks, offline JSON fallbacks, and update notifications
- updated search, RSS, sitemap, authoring scripts, validation, deployment docs, and the cache namespace
- removed duplicate source folders, the obsolete page template and renderer, and generated cache files
